...
Page({
  ...
  formSubmit: function(e) {
    let header_src = e.detail.userInfo.avatarUrl
    //数据校验
    var val = this.data.vender
    if (this.tip(val.name&&val.name.length >= 3, '公司名至少三个字')) 
        return
    if (this.tip(val.contacts&&val.contacts.length >= 2, '请填写正确的联系人名字')) 
      return
    if (this.tip(val.phone&&val.phone.length == 11, '请填写正确的手机号'))
      return;
    var addressV = this.data.address;
    if (this.tip(addressV.name && addressV.latitude && addressV.longitude, '请选择公司所在地址'))
      return;
    if (this.tip(val.addressDesc&&val.addressDesc.length > 5, '请输入地址详细描述(至少五个字)'))
      return;
    //提交信息  
    wx.showLoading({
      title: '正在提交信息',
